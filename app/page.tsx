      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 lg:gap-8">
        <header className="space-y-3">
          ...
          <p className="max-w-3xl text-sm text-slate-600 sm:text-base">
            Configure endpoint auth with confidence before running checks. This slice wires local form state +
            validation only (no network calls yet).
          </p>
        </header>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr_1fr] lg:items-start">
          <article className="rounded-2xl border border-surface-border bg-surface-card p-5 shadow-sm sm:p-6">
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">Auth flow is now wired and validated on-device.</p>
              </div>
            ...
            <form className="space-y-4" aria-label="Server setup form">
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50/80 p-3 text-xs text-amber-800">
                Wiring complete for mode: <span className="font-semibold">{authModeLabel[form.authMode]}</span>. Run
                execution is intentionally disabled in this slice.
              </div>
          ...
          <article className="rounded-2xl border border-surface-border bg-surface-card p-5 shadow-sm sm:p-6">
            ...
            >
              {isReady
                ? `✅ Ready (${passCount}/${readinessItems.length}) — configuration can proceed to execution slice.`
          ...
          <article className="rounded-2xl border border-surface-border bg-surface-card p-5 shadow-sm sm:p-6">
            ...
            <p className="mt-1 text-sm text-slate-600">No request execution yet. States remain intentionally designed.</p>
            ...
            <div className="mt-4 space-y-3">
              <section className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4">
                ...
                <p className="mt-1 text-xs text-slate-500">Execution report appears here in upcoming slices.</p>
              </section>
